...
This document covers motion execution in the `grasping_arm_control` package.

For calibration of the workspace file consumed by this node, see [workspace_creation.md](./workspace_creation.md).

## Features

`arm_control_node` owns all robot-motion details after a client submits a `MoveToPose` goal.
