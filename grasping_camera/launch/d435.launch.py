...
def launch_setup(context, params, param_name_suffix=''):
    ...
    # Load lifecycle nodes setting from YAML (installed with this package).
    # Default to a normal node if the file is missing.
    lifecycle_param_file = os.path.join(
        os.path.dirname(__file__), '..', 'config', 'global_settings.yaml'
    )
    use_lifecycle_node = False
    if os.path.exists(lifecycle_param_file):
        lifecycle_params = yaml_to_dict(lifecycle_param_file)
        use_lifecycle_node = bool(lifecycle_params.get("use_lifecycle_node", False))
    else:
        print(
            "\033[33mWarning: global_settings.yaml not found; launching realsense2_camera as a Normal ROS Node.\033[0m"
        )
